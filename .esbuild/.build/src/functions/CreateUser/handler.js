...
var NewUser = async (event) => {
  try {
    ...
    if (!event.body.accessKey || !event.body.consoleUser || !event.body.secretKey || !event.body.systemID || !event.body.groupNames || !event.body.policyNames) {
      return formatJSONResponse({
        error: "Bad request"
      }, 400);
    }
    ...
    if (accessKey == "" || consoleUser == "" || secretKey == "" || systemID == "" || groupNames == "" || policyNames == "") {
      return formatJSONResponse({
        error: "Bad Request"
      }, 400);
    } else {

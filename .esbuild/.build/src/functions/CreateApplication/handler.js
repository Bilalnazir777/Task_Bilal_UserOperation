...
// src/functions/CreateApplication/handler.ts
var handler_exports = {};
...
var formatJSONResponse = (response, status) => {
  return {
    statusCode: status,
...
var DynamoDB2 = process.env.IS_OFFLINE ? new AWS.DynamoDB.DocumentClient({
  region: "localhost",
  endpoint: "http://localhost:8000"
}) : new AWS.DynamoDB.DocumentClient({});
var savetodb = async (body) => {
  const db_response = await DynamoDB2.put(body).promise();
  return db_response;
};
...
var CreateApplication = async (event) => {
  try {
    ...
    if (!event.body.name || !event.body.subnet || !event.body.roles || !event.body.readPct || !event.body.readIOPs || !event.body.writeIOPs || !event.body.buckets) {
      return formatJSONResponse({
        error: "Bad request"
      }, 400);
    }
    const { name, subnet, roles, readPct, readIOPs, writeIOPs, buckets } = event.body;
    ...
    if (name == "" && subnet == "" && roles == "" && readPct == null && readIOPs == null && writeIOPs == null && buckets == "") {
      return formatJSONResponse({
        error: "Bad Request"
      }, 400);
    } else {
...
var main = middyfy(CreateApplication);
